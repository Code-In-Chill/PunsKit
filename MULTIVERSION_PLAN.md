...
- [x] Tạo Interface `PlatformAdapter`.
- [x] Triển khai `PlatformDetector` để xác định môi trường runtime.
- [x] Tách `BrigadierIntegration` hiện tại vào `PaperAdapter`.

### Giai đoạn 2: Refactor CommandManager
- [x] Thay đổi `CommandManager` để sử dụng `PlatformAdapter`.
- [x] Xây dựng `LegacyAdapter` (Fallback) cho Spigot/Bản cũ.
- [x] Kiểm tra khả năng Tab-completion trên cả 2 nền tảng.

### Giai đoạn 3: Tối ưu hóa Java 21
- [x] Sử dụng **Virtual Threads** (Project Loom) cho tất cả các bean được đánh dấu `@PAsync`.
- [x] Tối ưu hóa `BeanRegistry` bằng **Pattern Matching** để code sạch hơn.
